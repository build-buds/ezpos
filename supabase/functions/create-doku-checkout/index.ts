...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const baseUrl = env === "production" ? "https://api.doku.com" : "https://api-sandbox.doku.com";
    ...
    if (!dokuRes.ok) {
      console.error("DOKU error", dokuRes.status, dokuJson);
      return new Response(
        JSON.stringify({
    ...
    if (!paymentUrl) {
      console.error("No payment URL in DOKU response", dokuJson);
      return new Response(
    ...
    console.error("create-doku-checkout error", e);
    return new Response(
