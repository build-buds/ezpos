...
type CheckoutResponse = { url?: string; error?: string; invoice?: string };
...
const Pricing = () => {
  ...
  const handlePolar = async () => {
    ...
    try {
      ...
      else toast.error("Tidak ada URL checkout dari server.");
    } catch (error) {
  ...
  const handleDoku = async () => {
    ...
    try {
      ...
      if (result?.url) window.location.href = result.url;
      else toast.error("Tidak ada URL pembayaran dari DOKU.");
    } catch (error) {
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge className="bg-accent text-accent-foreground text-[10px] py-0 px-1.5 h-4">
                    Sandbox
                  </Badge>
                </div>
